...
def compute_simcse_knn(test_mrc_data, train_mrc_data, knn_num, test_index=None):
    sim_model = SimCSE("princeton-nlp/sup-simcse-roberta-base") #princeton-nlp/sup-simcse-roberta-base
        ...
        for key,value in test_mrc_data.items():
            ...
            if "|" in value:
                ...
                for label in labels:
                    ...
                    example_idx=example_idx+[train_sentence_index[label][int(i)] for i in top_index[0]]
                    example_value=example_value+[float(value) for value in top_value[0]]
    ...
    #     if sub_index != 0:
    #         continue
    #     context = test_mrc_data[idx_][0]
    #     labels = test_mrc_data[idx_][1].split("|")
    #
    #     embedding = sim_model.encode([context], batch_size=128, normalize_to_unit=True, keepdim=True, return_numpy=True)
    #
    #     for label in labels:
...
if __name__ == '__main__':
    ...
    # print(test_mrc_data)
    train_mrc_data = read_mrc_data(dir_="data/NCBI/dev_NCBI.json")
    test_mrc_data=read_ner_pre_train_data("data/NCBI/test_NCBI.json","data/NCBI/5shot_prompt2_gpt4_output1.csv")
    # print(train_mrc_data)
    index_, value_ = compute_simcse_knn(test_mrc_data=test_mrc_data, train_mrc_data=train_mrc_data, knn_num=5)
    # print(index_)
    write_file(dir_="data/NCBI/5shot_test.jsonl", data=index_)